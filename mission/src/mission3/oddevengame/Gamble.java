...
class Gamble {
	...
	public void betting() {
		for (int i = 0; i < 8; i++) {
			...
			Rival rival = new Rival((int)(player.money * Math.pow(1.2, i+1)));
			while((player.money > 0)&&(rival.money > 0)) {
				int maxBetting = Math.min(player.money, rival.money);
				System.out.print("배팅금액을 입력해주세요(배팅한도: " + (int)maxBetting + "): ");
				bettingMoney = Game.scanner.nextInt();
				...
				System.out.println("현재 " + name + "의 소지금은 " + (int)player.money + ", 상대방의 소지금은 " + rival.money + "입니다.");
				System.out.println();
			}
			...
			if(player.money <= 0) {
				...
				System.out.println(i+1 + "번째 스테이지에서 " + name +"의 소지금은 " + player.money + "입니다.");
				System.out.println();
		...
		System.out.println("최종 " + name +"의 소지금은 " + player.money + "입니다.");
	}
}